...
describe("Semaphore", () => {
  ...
  const wasmFilePath = `${__dirname}/../snark-artifacts/${treeDepth}/semaphore.wasm`;
  const zkeyFilePath = `${__dirname}/../snark-artifacts/${treeDepth}/semaphore.zkey`;
  ...
  describe("# createGroup", () => {
    ...
    it("Should not create a group if is not registry", async () => {
      const transaction = semaphoreContract
        .connect(signers[1])
        ["createGroup(uint256,uint256,address)"](
          groupId,
    ...
    it("Should not create a group with a custom Merkle tree root expiration if is not registry", async () => {
      const groupId = 2;
  ...
  describe("# updateGroupMerkleTreeDuration", () => {
    it("Should not update a group Merkle tree duration if the caller is not the group admin", async () => {
      const transaction = semaphoreContract.updateGroupMerkleTreeDuration(
        groupId,
        300
      );
    ...
    it("Should update the group Merkle tree duration", async () => {
      const transaction = semaphoreContract
        .connect(signers[1])
        .updateGroupMerkleTreeDuration(groupId, 300);
  ...
  describe("# updateMember", () => {
    it("Should not update a member if the caller is not the group admin", async () => {
      ...
      const transaction = semaphoreContract
        .connect(signers[1])
        .updateMember(groupId, member, 1, [0, 1], [0, 1]);
    ...
    it("Should update a member from an existing group", async () => {
      ...
      const transaction = semaphoreContract.updateMember(
        groupId,
        BigInt(1),
        BigInt(4),
        siblings,
  ...
  describe("# removeMember", () => {
    it("Should not remove a member if the caller is not the group admin", async () => {
      ...
      const transaction = semaphoreContract
        .connect(signers[1])
        .removeMember(groupId, member, [0, 1], [0, 1]);
    ...
    it("Should remove a member from an existing group", async () => {
      ...
      const transaction = semaphoreContract.removeMember(
        groupId,
        BigInt(3),
        siblings,
        pathIndices
      ...
      await expect(transaction)
        .to.emit(semaphoreContract, "MemberRemoved")
        .withArgs(groupId, 2, BigInt(3), root);
    });
  });
  ...
  describe("# updateRegistry", () => {
    it("Should change registry and able to create a group", async () => {
      const groupId = 10;
      const group = new Group(groupId);
      await semaphoreContract
        .connect(signers[0])
        ["updateRegistry(address)"](accounts[2]);
      const transaction = semaphoreContract
        .connect(signers[2])
        ["createGroup(uint256,uint256,address)"](
          groupId,
      ...
      await expect(transaction)
        .to.emit(semaphoreContract, "GroupAdminUpdated")
        .withArgs(groupId, constants.AddressZero, accounts[2]);
    });
  });
});
