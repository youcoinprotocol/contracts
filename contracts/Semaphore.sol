...
/// @title Semaphore
/// @dev This contract is derived from Semaphore base contracts
...
contract Semaphore is ISemaphore, SemaphoreGroups {
    ...
    /// @dev Only allow registry address to perform group creation
    modifier onlyRegistry() {
        if (registryAddress != _msgSender()) {
            revert NotRegistry();
        }
        _;
    }
    ...
    constructor(ISemaphoreVerifier _verifier, address _registryAddress) {
        verifier = _verifier;
        registryAddress = _registryAddress;
    }

    /// @dev Updates registry address
    function updateRegistry(address _registryAddress) external override onlyRegistry {
        registryAddress = _registryAddress;
    }
